...
app.controller("listadehabitos", ["$scope", "$http", "habitos", function($scope, $http, habitos){
  ...
  if (habitos.habitos.length == 0){
    $http.get('http://localhost:80/listadehabitos-rest-api/habito.php').success(function(data) {
...
$scope.vencerHabito = function(habito){
  ...
  habito.status = "V";
  $http.put('http://localhost:80/listadehabitos-rest-api/habito.php', habito).success(function(data){
...
app.controller("novohabito", ["$scope", "$http", "habitos", function($scope, $http, habitos){
  ...
  $http.post('http://localhost:80/listadehabitos-rest-api/habito.php', {
    nome: nome
  }).success(function(data) {
